## Working Agreement

# Working Hours

Write your working hours/days and initials to sign the working agreement.

Fergus
* 1:00pm-8:00pm Mon-Fri (On days with lectures, it will be 9:00am-5:00pm)
* FL

Cole
* I’ll say when I get my work rota when I will be at work during the week and will work between 11 and 8 where I am available
* CP
* WC 27th Jan - working wednesday 29th from 1700
* WC 3rd Feb - free (friday unknown)

Diyorbek
* Monday – Friday (1pm – 8pm) Weekends too if need
* DS

Connor
* Monday – Friday 11:00 – 20:00
* CJ

Beau
* Sunday-Saturday 12:00 - 20:00
* select days will be given with 3 days notice via teams chat
* BM

Code Quality
* Code will be pushed to a separate feature branch
* Code will be maintainable, clear and well-documented
* AI generated code will be tested to make sure it fits our architecture, code style, and functionality
* Dependencies only added if necessary, and discussed with team
* Commit messages and comments are professional

Workflow
* Code will be uploaded to online repository regularly, so that progress can be seen
* Team members will work on issues that are prioritised
* Issues that are not prioritised will be worked on if there is no progress to be made on prioritised issues
* Blockers for issues will be raised immediately if they cannot be solved
* Merge requests will follow the git flow: rebasing, squashing commits, merging
* Related features in development will be collaborated on
* Draft prefix is only removed once merge request is ready (including squashed commits)
* When resolving merge conflicts, extreme care will be taken to avoid disrupting or discarding others’ work

Review
* Merge requests will go through 2 reviews
* Reviews will be thorough
* Reviewers will follow all testing instructions and follow review checklists
* Reviewers will be assigned weekly
* If you are assigned a review, the review will be completed within 48 hours, or notice given

Meetings
* Team members will attend all meetings that they have agreed to
* Any issues blocking development will be raised during meetings
* Meetings will be recorded or transcribed

General Working Environment
* Disagreements will be handled respectfully
* Ideas reviewed critically by the team
* Alternative ideas will be documented, raised and discussed
